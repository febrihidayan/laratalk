...
.mixin({
    ...
    methods: {
        ...
        getTransMessage(item) {
            ...
            if (_type === _.chat) {
                return this._setVal(
                    this.laratalk.profile.id !== item.content_by
                        ? item.user_by.name+': ' : '',
                    item.content
                )
            }
            ...
            if (_type === _.create_group) {
                return myUser
                    ? _t.create_group_by
                    : this._setVal(item.user_by.name, _t.create_group)
            }

            // type: 2
            if (_type === _.avatar_group) {
                return this._setVal(item.user_by.name, _t.avatar_group)
            }
            ...
            if (_type === _.description_group) {
                return this._setVal(item.user_by.name, _t.description_group)
            }

            // type: 5
            if (_type === _.info_all_group) {
                return this._setVal(item.user_by.name, _t.info_all_group)
            }

            // type: 6
            if (_type === _.info_admin_group) {
                return this._setVal(item.user_by.name, _t.info_admin_group)
            }

            // type: 7
            if (_type === _.chat_all_group) {
                return this._setVal(item.user_by.name, _t.chat_all_group)
            }

            // type: 8
            if (_type === _.chat_admin_group) {
                return this._setVal(item.user_by.name, _t.chat_admin_group)
            }

            // type: 9
            if (_type === _.add_user_group) {
                if (myUser) {
                    return this._setVal(_t.add_user_group_by, item.user_to.name)
                }
                if (this.laratalk.profile.id === item.user_to.id) {
                    return this._setVal(item.user_by.name, _t.add_user_group_to)
                }
            ...
            if (_type === _.remove_user_group) {
                if (myUser) {
                    return this._setVal(_t.remove_user_group_by, item.user_to.name)
                }
                if (this.laratalk.profile.id === item.user_to.id) {
                    return this._setVal(item.user_by.name, _t.remove_user_group_to)
                }
            ...
            // type: 11
            if (_type === _.add_admin_group && this.laratalk.profile.id === item.user_to.id) {
                return _t.add_admin_group
            }

            // type: 12
            if (_type === _.remove_admin_group && this.laratalk.profile.id === item.user_to.id) {
